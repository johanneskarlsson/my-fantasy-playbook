...
const _runtimeConfig = {"app":{"baseURL":"/","buildAssetsDir":"/_nuxt/","cdnURL":""},"nitro":{"routeRules":{"/__nuxt_error":{"cache":false}},"envPrefix":"NUXT_"},"public":{"YAHOO_APP_ID":"nkB6cX29","YAHOO_CLIENT_ID":"dj0yJmk9R0xnTXA1VmlERThsJmQ9WVdrOWJtdENObU5ZTWprbWNHbzlNQT09JnM9Y29uc3VtZXJzZWNyZXQmc3Y9MCZ4PWNm","YAHOO_CLIENT_SECRET":"999c9a68460f6d59fae2f0f2f425b45d9c56f901","APP_URL":"localhost:3000"},"proxy":{"options":{"target":"https://wide-onions-share-81-234-67-16.loca.lt/","changeOrigin":true,"pathRewrite":{"^/api/todos":"/todos","^/api/users":"/users"},"pathFilter":["/api/todos","/api/users"]}}};
...
app.get("/yahoo/league/standings", (req, res) => {
  app.yf.league.standings("nhl.l.17177").then((data) => {
    res.status(200).send(data);
  }).catch((err) => {
    res.status(400).send(err);
  });
});
const _kHLcT2 = fromNodeMiddleware(app);

function createProxyMiddleware(options) {
  const proxyMiddleware = createProxyMiddleware$1(options);
...
const buildtimeOptions = {"target":"https://wide-onions-share-81-234-67-16.loca.lt/","changeOrigin":true,"pathRewrite":{"^/api/todos":"/todos","^/api/users":"/users"},"pathFilter":["/api/todos","/api/users"]};
        const runtimeOptions = [].concat(useRuntimeConfig().proxy?.options)[0];
    
        const _6FAvnH = createProxyMiddleware(defu$1(runtimeOptions, buildtimeOptions));
...
const handlers = [
  ...
  { route: '/**', handler: _lazy_4xVOTT, lazy: true, middleware: false, method: undefined }
];
